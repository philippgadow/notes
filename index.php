  ...
  <body>
    ...
    <div class='container'>
        ...
        <?php
          ...
          $files = array();
          foreach (scandir($relPath) as $file) {
          // pick all $fileType files
            if (substr($file, -strlen($fileType)) === $fileType) {
              $files[$file] = filemtime($relPath . $file);
            }
          }
          arsort($files);
          ...
          foreach ($files as $file) {
            ...
            $displayPath = "displayNote.php?file=" . $file;
            $row = array("name"=>$file, "modified"=>date("d F Y H:i:s",filemtime($relPath . $file)), "displayPath"=>$displayPath);
